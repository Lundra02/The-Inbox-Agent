...
router.patch("/:sku", async (req, res) => {
  ...
  if (!Number.isSafeInteger(version) || version < 0 || !validateStockUpdate(changes)) return res.status(400).json({ error: "Use non-negative stock quantities, a valid price, arrival date and stock version." });
  try {
    const item = await InventoryItem.findOneAndUpdate({ sku: req.params.sku, __v: version }, { $set: changes, $inc: { __v: 1 } }, { new: true, runValidators: true }).lean();
